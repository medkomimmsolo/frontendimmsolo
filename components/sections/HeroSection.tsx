                  ...
                  displayEvents.map((event) => (
                    ...
                    <div className="flex gap-4 items-center">
                      <div className="shrink-0 text-center w-[68px] bg-[#0a0f1a]/80 rounded-sm py-3 border border-white/5 shadow-inner transition-colors group-hover:border-[#c20000]/30" suppressHydrationWarning>
                        <div className="text-[10px] text-[#ff4d4d] font-bold uppercase tracking-widest mb-1 leading-none" suppressHydrationWarning>{new Date(event.event_date).toLocaleDateString('id-ID', { month: 'short', timeZone: 'Asia/Jakarta' })}</div>
                        <div className="text-2xl font-black text-white leading-none" suppressHydrationWarning>{new Intl.DateTimeFormat('id-ID', { day: '2-digit', timeZone: 'Asia/Jakarta' }).format(new Date(event.event_date))}</div>
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-col gap-1.5 text-xs text-white/60 font-medium">
                          ...
                          <div className="flex items-center gap-1.5" suppressHydrationWarning><Clock className="w-3.5 h-3.5 shrink-0 text-[#c20000]"/> {new Date(event.event_date).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Jakarta' }).replace(/\./g, ':')} WIB</div>
                        </div>
                      </div>
                      </div>
                    </Link>
